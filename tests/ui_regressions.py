...
with sync_playwright() as p:
 ...
 def load(name):
  ...
  html=(BUILD/f'{name}.html').read_text();html=re.sub(r'<script[^>]*>.*?</script>','',html);html=re.sub(r'<link[^>]*>','',html)
...
(ROOT/'ui-test-results.json').write_text(json.dumps({'api_backend':'Mocked Chrome API, actual compiled worker and UI code','results':results},indent=2))
